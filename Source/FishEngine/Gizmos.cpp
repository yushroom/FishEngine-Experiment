...
static Shader* solidColorShader = nullptr;

static Texture* cameraGizmoTexture = nullptr;
static Texture* lightGizmoTexture = nullptr;
...
const char* SolidColorShaderStr = R"(
...
void Gizmos::Init()
{
	...
	s_circleMesh = new SimpleMesh(circle_vertex, circleVertexCount, GL_LINE_LOOP);
	...
	s_boxMesh = new SimpleMesh(box_vertex, 24, GL_LINES);
	...
	s_light = new SimpleMesh(lightGizmoVertex, circleVertexCount + numLines * 2, GL_LINES);
...
//    s_wiredSphereMesh = std::make_shared<SimpleMesh>(wiredSphereVertex, circleVertexCount*3, GL_LINE_STRIP);
	...
	solidColorShader = Shader::FromString(SolidColorShaderStr);
...
void Gizmos::DrawLine(const Vector3& from, const Vector3& to)
{
	...
	auto const & view = Camera::GetMainCamera()->GetWorldToCameraMatrix();
	auto const & proj = Camera::GetMainCamera()->GetProjectionMatrix();
	shader->Use();
	shader->BindUniform("MATRIX_MVP", proj*view*s_matrix);
	...
	const float         radius)
{
	...
	for (int i = 0; i < 3; ++i)
	{
		...
		shader->BindUniform("MATRIX_MVP", proj*view*s_matrix*m);
		s_circleMesh->Render();
	}
}
