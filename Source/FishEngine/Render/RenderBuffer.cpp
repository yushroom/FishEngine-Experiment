...
ColorBuffer* ColorBuffer::Create(int width, int height, TextureFormat  format /*= TextureFormat::RGBA32*/)
{
	auto t = new ColorBuffer();
...
DepthBuffer* DepthBuffer::Create(const int width, const int height)
{
	auto t = new DepthBuffer();
	t->m_dimension = TextureDimension::Tex2D;
	t->m_width = width;
	t->m_height = height;
	glGenTextures(1, &t->m_GLNativeTexture);
...
LayeredDepthBuffer* LayeredDepthBuffer::Create(const int width, const int height, const int depth, bool useStencil /*= true*/)
{
	auto t = new LayeredDepthBuffer();
